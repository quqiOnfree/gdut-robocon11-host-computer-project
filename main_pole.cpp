...
int main(int argc, char *argv[]) {
  ...
  try {
    // ---- 参数 ----
    const char *env_model = std::getenv("POLE_MODEL_PATH");
    std::string modelPath =
        env_model ? env_model : "model/pole_detect.onnx";
    ...
    YoloOnnxDetector detector(modelPath, {"pole"});
    std::cout << "模型输入: " << detector.inputW() << "x"
              << detector.inputH() << "\n";
    ...
    while (true) {
      ...
      // 检测
      auto dets = detector.process(frame, confThres);

      // 找离画面中心最近的 pole
      const auto *best = YoloOnnxDetector::nearestToCenter(dets, frame.size());
      cv::Point pole_center = best ? best->center() : cv::Point(0, 0);
