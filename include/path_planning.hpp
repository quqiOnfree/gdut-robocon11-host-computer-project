...
public:
  ...
  struct a_star_node {
    ...
    std::array<std::array<bool, map_height>, map_width> walked{}; // To track walked nodes
    std::size_t f_cost() const { return g_cost + h_cost; } // Total cost
  };

  std::queue<a_star_node> find_path(const point &start, const point &end) const {
...
protected:
  ...
  struct compare_a_star_node {
    bool operator()(const std::queue<a_star_node> &a,
                    const std::queue<a_star_node> &b) const {
      return a.back().f_cost() > b.back().f_cost();
    }
  };

  std::queue<a_star_node> a_star(const point &start, const point &end) const {
    std::priority_queue<std::queue<a_star_node>,
                        std::vector<std::queue<a_star_node>>,
                        compare_a_star_node>
        path;
    {
      // 初始化路径
      std::queue<a_star_node> initial_path;
    ...
    while (!path.empty()) {
      std::queue<a_star_node> current_path = path.top();
      path.pop();
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        case 0: // Up
        {
          ...
          a_star_node next_node{next_point, next_type,
          ...
          std::queue<a_star_node> new_path = current_path;
