...
// YOLO end2end 检测器，专用于 weapon + ITF 联合检测
// 输出格式: [1, N, 6] — 每行 [x1, y1, x2, y2, score, class_id]
class YoloOnnxWeapon : public YoloOnnxProcessor<YoloOnnxWeapon> {
  friend class YoloOnnxProcessor<YoloOnnxWeapon>;
...
public:
  ...
  YoloOnnxWeapon(const std::filesystem::path &modelPath,
                 const std::vector<std::string> &classNames)
      : YoloOnnxProcessor<YoloOnnxWeapon>(modelPath), classNames_(classNames) {}
  ...
  static void drawDetections(cv::Mat &image,
    ...
    for (const auto &det : detections) {
      ...
      std::string label =
          det.className + ": " + cv::format("%.2f", det.score);
